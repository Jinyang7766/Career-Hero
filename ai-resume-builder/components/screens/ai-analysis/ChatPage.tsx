  ...
  return (
    <div className="fixed inset-0 mx-auto w-full max-w-md z-[100] bg-slate-50 dark:bg-[#0b1219] flex flex-col animate-in slide-in-from-right duration-300 overflow-hidden pt-[76px]">
      ...
      <ChatPageHeader
        ...
        onInterruptThinking={onInterruptThinking}
      ...
      {isInterviewMode && (
        <InterviewProgressCard
          ...
          currentQuestionElapsedSec={currentQuestionElapsedSec}
          isSending={isSending}
          isRecording={isRecording}
      ...
      <div
        ref={inputBarRef}
        className="fixed left-0 right-0 mx-auto w-full max-w-md bottom-0 z-[100] px-4 py-3 bg-white/80 dark:bg-[#1c2936]/80 backdrop-blur-lg border-t border-slate-200 dark:border-white/5"
        style={{
          ...
          <div className="flex gap-3 items-end">
            ...
            {inputMode === 'voice' ? (
              <div className="flex-1 relative">
                {isRecording && (
                  <div className="fixed inset-0 mx-auto w-full max-w-md z-[105] bg-gradient-to-t from-black/80 via-black/40 to-transparent animate-in fade-in duration-300 pointer-events-none" />
                )}

                <div
                  className={`relative flex flex-col items-center transition-all duration-300 ${isRecording
                    ? 'fixed left-0 right-0 mx-auto w-full max-w-md px-4 bottom-[calc(max(12px,env(safe-area-inset-bottom))+12px)] z-[110]'
                    : 'relative'
                    }`}
                >
                  {isRecording && (
