          ...
          <div className="space-y-4 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {filteredRows.map((row) => {
              ...
              return (
                <div
                  key={String(row.id)}
                  className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md transition-all active:scale-[0.985] group"
                >
                  <div className={`size-12 rounded-2xl shrink-0 flex items-center justify-center transition-colors
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <p className={`text-lg font-black tracking-tight ${isGain ? 'text-emerald-500' : 'text-rose-500'}`}>
                        {isGain ? `+${row.delta}` : row.delta}
                      </p>
                    </div>

                    <div className="flex items-center justify-between">
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate pr-4 font-medium opacity-80">
                        {row.note || (isGain ? '积分发放' : '积分扣减')}
                      </p>
                      <div className="flex items-center gap-2 shrink-0">
                        {row.balance_after != null && Number.isFinite(Number(row.balance_after)) && (
                          <div className="px-1.5 py-0.5 rounded-md bg-slate-50 dark:bg-white/5 text-[9px] font-bold text-slate-400 dark:text-slate-500">
