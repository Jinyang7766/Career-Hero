...
4. 字段一致性回归自动化（已完成，已从剩余项移出）：
   - 已补齐“编辑态 -> 存储 -> 总览展示”字段一致性回归用例（覆盖 MBTI/性别/求职意向等高频字段）。
   - 已补齐“同值不重复落多字段”与“总结区派生内容只读守卫”回归断言。
   - 已补齐“派生数据不反写主字段”回归断言，避免历史数据回灌。
5. 画像主字段校验灰度观测收口（当前唯一待完成）：
   - 基于已上线 Schema 校验日志（count/paths/types）补齐外部平台看板与告警阈值。

说明：
- “画像编辑器与简历编辑器第二阶段融合”已取消，不再作为 P1 待办。
- “画像入库 Schema 校验加固（主字段优先）”已在 v6.27 完成代码与测试闭环。

### 10.2.1 计划书状态清单（按模块/文件，含已收口与剩余项）
...
**P1（随后一周，剩余未完成）**
1. 主字段 Schema 校验灰度观测（外部平台接线）：
   - 模块：日志平台索引/看板/告警规则（代码侧已在 P0 收口）。
   - 范围：基于 `count/paths/types` 观测字段完成灰度看板与阈值告警上线。

说明：
- `Step2 定向追问卡片产品化` 已完成（`19ce78c`），不再列入未完成。
- `Step5 精修产品化收口` 已完成（`e8b3b5e`），不再列入未完成。
- `字段一致性与派生兼容回归` 已完成（见 10.5 第 3 条），不再列入未完成。
