...
### v1.3.1 (2026-02-17) - 面试体验与 UI 精细化 ✨
- **🎨 面试进度条重构**：
  - 采用**毛玻璃 (Glassmorphism)** 拟态设计，移除生硬边框，增加流光渐变与呼吸灯动效。
  - 实现了**平滑加载状态**：在题库生成期间即时展示热身题，进度条显示“正在加载面试题库”动画，彻底告别空白等待。
- **⚡ 智能热身逻辑**：
  - **零延迟启动**：进入面试立即加载首个热身问题（自我介绍/成就深挖），无需等待 AI 生成完整题单。
  - **无缝衔接**：后台静默加载后续题库，完成后自动合入进度条，并保持当前会话流畅。
- **💎 全局交互统一度**：
  - 废弃了浏览器原生 `confirm` 弹窗，全站统一使用**自定义高颜值模态框**（支持深色模式），提升操作质感。
